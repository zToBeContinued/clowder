  ...
  for (let i = 1; i < msgs.length; i++) {
    ...
    for (let j = Math.max(0, i - 6); j < i; j++) {
      ...
      if (a.content.slice(0, PREFIX_LEN) === b.content.slice(0, PREFIX_LEN)) {
        ...
        console.log(
          `  原消息: id=${a.id} cat=${a.catId || `(user:${a.userId})`} ts=${new Date(a.ts).toISOString()} len=${a.content.length}`,
        );
        console.log(
          `  复读方: id=${b.id} cat=${b.catId} ts=${new Date(b.ts).toISOString()} len=${b.content.length} (相隔 ${Math.round((b.ts - a.ts) / 1000)}s)`,
        );
