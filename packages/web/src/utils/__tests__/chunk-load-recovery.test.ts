...
describe('chunk-load-recovery', () => {
  ...
  it('recognizes resource-tag load failures via event.target src/href (script/link 404)', () => {
    ...
    const scriptFailure = {
      type: 'error',
      target: { src: 'http://localhost:3003/_next/static/chunks/settings-abc123.js' },
    };
    ...
    expect(
      isRecoverableChunkLoadError({ type: 'error', target: { src: 'http://localhost:3003/avatars/cat.png' } }),
    ).toBe(false);
  });
