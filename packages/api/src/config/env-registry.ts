...
export const ENV_VARS: EnvDefinition[] = [
  ...
  },
  {
    ...
    defaultValue: '0（永不过期）',
    description:
      '消息过期时间（秒）。不设或设为 0/负数 → 消息永不过期（RedisMessageStore 的 DEFAULT_TTL_SECONDS = 0）。填正数才启用 EXPIRE。注意：过期的 Redis 消息不影响已索引的 evidence_passages（Phase I 保证永久性）。',
    category: 'storage',
    sensitive: false,
  },
  {
    ...
    defaultValue: '0（永不过期）',
    description: '对话过期时间（秒）。不设或设为 0/负数 → 永不过期（RedisThreadStore 的 DEFAULT_TTL = 0）。',
    category: 'storage',
    sensitive: false,
  },
  {
    ...
    defaultValue: '0（永不过期）',
    description: '任务过期时间（秒）。不设或设为 0/负数 → 永不过期（RedisTaskStore 的 DEFAULT_TTL = 0）。',
    category: 'storage',
    sensitive: false,
  },
  {
    ...
    defaultValue: '0（永不过期）',
    description: '摘要过期时间（秒）。不设或设为 0/负数 → 永不过期（RedisSummaryStore 的 DEFAULT_TTL = 0）。',
    category: 'storage',
    sensitive: false,
  },
  {
  ...
  },
  {
    ...
    defaultValue: '300（5分钟）',
    description:
      '流式草稿过期时间（秒）。不设 → 300 秒（RedisDraftStore 的 DEFAULT_TTL = 300，是本组 TTL 里唯一默认就过期的）。设为 0/负数 → 永不过期。',
    category: 'storage',
    sensitive: false,
  },
  {
  ...
  {
    name: 'CAT_CAFE_A2A_MAX_TARGETS',
    defaultValue: '3',
    description: '单条消息最多可 @ 的猫数（A2A 文本接力解析上限）',
    category: 'a2a',
    sensitive: false,
  },
  {
    name: 'CAT_CAFE_A2A_PINGPONG_LIMIT',
    defaultValue: '4',
    description: '同一对猫无实质内容互相 @ 的熔断轮数（长内容/实质工具调用豁免）',
    category: 'a2a',
    sensitive: false,
  },
  {
    name: 'CAT_CAFE_A2A_PINGPONG_WARN',
    defaultValue: '2',
    description: '乒乓熔断前的警告轮数',
    category: 'a2a',
    sensitive: false,
  },
  {
  ...
  {
    name: 'CAT_CAFE_ACP_POOL_MAX_PROCESSES',
    defaultValue: '3',
    description: 'ACP 进程池上限（Kiro 不支持多路复用，此值即 Kiro 猫跨 channel 并发上限）',
    category: 'server',
    sensitive: false,
  },
  {
