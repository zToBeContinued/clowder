...
export class RedisTaskStore implements ITaskStore {
  ...
  private serializeTask(task: TaskItem): Record<string, string> {
    ...
    if (task.events) {
      out.events = JSON.stringify(task.events);
    }
    return out;
  }
  ...
  private hydrateTask(data: Record<string, string>): TaskItem {
    const base: TaskItem = {
      ...
      failureClass: (data.failureClass || undefined) as TaskItem['failureClass'],
    ...
    if (data.events) {
      try {
        ...
        task = { ...task, events: Array.isArray(events) ? events : [] };
      } catch {
        // Ignore corrupted optional event payloads.
      }
    }
    return task;
  }
