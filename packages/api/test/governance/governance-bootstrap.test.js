...
describe('GovernanceBootstrapService', () => {
  ...
  describe('state-only mode (default)', () => {
    it('writes NOTHING into the project tree by default', async () => {
      ...
      // Project tree must be completely untouched
      const { readdir } = await import('node:fs/promises');
      const entries = await readdir(targetProject);
      assert.deepStrictEqual(entries, [], 'project tree must stay empty in state-only mode');
    });

    it('registers project with writeMode state-only', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      await svc.bootstrap(targetProject, { dryRun: false });

      const entry = await svc.getRegistry().get(targetProject);
      assert.ok(entry);
      assert.equal(entry.confirmedByUser, true);
      assert.equal(entry.writeMode, 'state-only');
    });

    it('skips registry rewrite when already up to date (no churn)', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      await svc.bootstrap(targetProject, { dryRun: false });
      const first = await svc.getRegistry().get(targetProject);

      await new Promise((r) => setTimeout(r, 10));
      await svc.bootstrap(targetProject, { dryRun: false });
      const second = await svc.getRegistry().get(targetProject);

      assert.equal(second.syncedAt, first.syncedAt, 'up-to-date entry must not be rewritten');
    });
  });

  describe('full mode (explicit opt-in)', () => {
    it('bootstraps empty project with all governance files', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      const report = await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full' });
    ...
    it('managed block contains the Scope Guard for non-Clowder sessions', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full' });

      const content = await readFile(join(targetProject, 'AGENTS.md'), 'utf-8');
      assert.ok(content.includes('Scope Guard'), 'block must open with the scope guard');
      assert.ok(content.includes('CAT_CAFE_CAT_ID'), 'guard must reference the session env marker');
      assert.ok(
        content.includes('you are NOT a Cat Cafe agent'),
        'guard must tell non-Clowder sessions to ignore the block',
      );
    });

    it('creates per-skill symlinks for all 4 providers (ADR-025, skillTier all)', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      ...
      for (const dir of ['.claude/skills', '.codex/skills', '.gemini/skills', '.kimi/skills']) {
        ...
        for (const skill of ['tdd', 'worktree', 'quality-gate']) {
          ...
          assert.equal(
            ...
            `${dir}/${skill} should point to cat-cafe-skills/${skill}`,
          );
        }
    ...
    it('does not create skill symlinks with default skillTier none', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full' });

      for (const dir of ['.claude/skills', '.codex/skills']) {
        await assert.rejects(lstat(join(targetProject, dir, 'tdd')), { code: 'ENOENT' });
      }
    });
    ...
    it('is idempotent — second run produces no created actions', async () => {
      ...
      const report2 = await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full' });
    ...
    it('dry-run writes nothing to disk', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      const report = await svc.bootstrap(targetProject, { dryRun: true, writeMode: 'full' });
    ...
    it('registers project in governance registry with writeMode full', async () => {
      const svc = new GovernanceBootstrapService(catCafeRoot);
      await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full' });
      ...
      assert.equal(entry.confirmedByUser, true);
      assert.equal(entry.writeMode, 'full');
    });
    ...
    it('skips symlink if already correct', async () => {
      ...
      const report2 = await svc.bootstrap(targetProject, { dryRun: false, writeMode: 'full', skillTier: 'all' });
